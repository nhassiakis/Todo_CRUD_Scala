...
class TodoController @Inject()(
    ...
    todoService: TodoService) extends  AbstractController(cc) {
        ...
        def add() = Action.async { implicit request: Request[AnyContent] =>
            ...
            data => {
                val newTodoItem = Todo(0, data.name, data.isComplete, data.createdAt, data.completedAt)
                todoService.addItem(newTodoItem).map(item => Redirect(routes.TodoController.getAll))
            })
        }
        ...
        TodoForm.form.bindFromRequest.fold(
          ...
          data => {
            val todoItem = Todo(id, data.name, data.isComplete, data.createdAt, data.completedAt)
